...
from second.data.nuscene_my.utils.data_classes import LidarPointCloud, RadarPointCloud, Box
from second.data.nuscene_my.utils.geometry_utils import view_points, box_in_image, BoxVisibility
...
class NuScenes:
    ...
    def render_sample_data(self, sample_data_token: str, with_anns: bool = True,
        ...
        self.explorer.render_sample_data(sample_data_token, with_anns, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
                                         out_path=out_path)
    def render_sample_data_3view(self, sample_data_token: str, with_anns: bool = True,
                           box_vis_level: BoxVisibility = BoxVisibility.ANY, axes_limit: float = 40, ax: Axes = None,
                           nsweeps: int = 1, out_path: str = None) -> None:
        self.explorer.render_sample_data_3view(sample_data_token, with_anns, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
                                         out_path=out_path)
...
class NuScenesExplorer:
    ...
    def render_sample_data(self,
        ...
        if out_path is not None:
            plt.savefig(out_path)
    def render_sample_data_3view(self,
                           sample_data_token: str,
                           with_anns: bool = True,
                           box_vis_level: BoxVisibility = BoxVisibility.ANY,
                           axes_limit: float = 40,
        ...
        if sensor_modality == 'lidar':
            ...
            colors0 = np.minimum(1, np.sqrt(np.sum(pc.points[:2, :] ** 2, axis=0))/axes_limit/np.sqrt(2))
            colors1 = np.minimum(1, np.sqrt(np.sum(pc.points[[0,2], :] ** 2, axis=0))/axes_limit/np.sqrt(2))
            colors2 = np.minimum(1, np.sqrt(np.sum(pc.points[[1,2], :] ** 2, axis=0))/axes_limit/np.sqrt(2))
            axes[0].scatter(points[0, :], points[1, :], c=colors0, s=0.2)
            axes[1].scatter(points[0, :], points[2, :], c=colors1, s=0.2)
            axes[2].scatter(points[1, :], points[2, :], c=colors2, s=0.2)
            # Show ego vehicle.
            axes[0].plot(0, 0, 'x', color='black')
            axes[1].plot(0, 0, 'x', color='black')
            axes[2].plot(0, 0, 'x', color='black')
            # Show boxes.
            if with_anns:
                for box in boxes:
                    c = np.array(self.get_color(box.name)) / 255.0
                    box.render(axes[0], view=np.eye(4), colors=(c, c, c))
                    box.render(axes[1],  view=np.eye(4), colors=(c, c, c),viewsight='front')
                    box.render(axes[2],  view=np.eye(4), colors=(c, c, c),viewsight='side')
            # Limit visible range.
            axes[0].set_xlim(-axes_limit, axes_limit)
            axes[0].set_ylim(-axes_limit, axes_limit)
            axes[1].set_xlim(-axes_limit, axes_limit)
            axes[1].set_ylim(-10, 10)
            axes[2].set_xlim(-axes_limit, axes_limit)
